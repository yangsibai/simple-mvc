...
module.exports = (options, parentApp) ->
    ...
    configureDir(options, dir, parentApp)

configureDir = (options, dirName, parentApp, parentDir) ->
    fs.readdirSync(dirName).forEach (fileName) ->
        filePath = path.join(dirName, fileName)
        parentDir = parentDir or ""
        if fs.lstatSync(filePath).isDirectory()
            configureDir(options, filePath, parentApp, path.join(parentDir, fileName))
        else if path.extname(fileName) is ".js"
            controllerName = path.join(parentDir, path.basename(fileName, path.extname(fileName)))
            configureFile(options, parentApp, controllerName)
