...
module.exports = function (parent) {
    ...
    fs.readdirSync(path.join(PROJECT_DIR, '/controllers')).forEach(function (name) {
        var controllerName = name.replace('.js', ''); //去掉后缀名
        var controller = require(path.join(PROJECT_DIR, 'controllers', controllerName));

        var app = express();
        app.set('views', path.join(PROJECT_DIR, 'views', controllerName));
